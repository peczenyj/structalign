...
## Build provenance (SLSA Build L1)

Release binaries are built by a scripted, consistent process — [GoReleaser](https://goreleaser.com)
driven by a tagged commit through `.github/workflows/release.yml` — and each
release artifact is published with a signed [SLSA](https://slsa.dev) build
provenance attestation generated by GitHub's `attest-build-provenance` action.
The provenance records how and from what source the artifact was built, meeting
[SLSA Build Level 1](https://slsa.dev/spec/v1.0/levels#build-l1).
